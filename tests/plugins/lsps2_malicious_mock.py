#!/usr/bin/env python3
"""
Malicious LSPS2 mock for security testing
=========================================

Like lsps2_service_mock.py but can be configured to send:
- Inflated extra_fee TLV (exceeds opening_fee) via malicious_extra_fee_msat
- Per-part inflated fee via malicious_per_part_fee_msat

This is used to test that the LSPS2 client properly validates and rejects
malicious extra_fee values per LSPS2 spec requirements.
"""
...
def lsps2_policy_getchannelcapacity(
    ...
    """Returns channel capacity for the LSPS2 plugin."""
...
class State:
    ...
    # Malicious mode options
    malicious_extra_fee_msat: Optional[int] = None  # Override total fee to send
    malicious_per_part_fee_msat: Optional[int] = None  # Override per-part fee
    ...
    for _ in range(120):
        ...
        for c in channels:
            if c.get("state") == "CHANNELD_NORMAL":
                plugin.log("zero-conf channel is NORMAL; releasing HTLCs")
                return True
        time.sleep(1)
    return False
...
def _modify_payload_and_build_response(held: Held):
    ...
    payload = None
    extra = None
    if amt_msat != forward_msat or state.malicious_extra_fee_msat or state.malicious_per_part_fee_msat:
        # Build modified payload with reduced forward amount
        amt_byte = struct.pack("!Q", forward_msat)
        while len(amt_byte) > 1 and amt_byte[0] == 0:
            amt_byte = amt_byte[1:]
        onion_payload = TlvPayload().from_hex(held.onion["payload"])
        p = TlvPayload()
        p.add_field(2, amt_byte)
        p.add_field(4, onion_payload.get(4).value)
        p.add_field(6, onion_payload.get(6).value)
        payload = p.to_bytes(include_prefix=False)

        # ATTACK MODE: Determine fee to advertise in TLV
        fee_to_send = fee_applied
        if state.malicious_extra_fee_msat is not None:
            # Override with inflated total fee
            fee_to_send = state.malicious_extra_fee_msat
            plugin.log(f"MALICIOUS: Sending inflated extra_fee={fee_to_send} (actual={fee_applied})")
        elif state.malicious_per_part_fee_msat is not None:
            # Override with per-part inflated fee
            fee_to_send = state.malicious_per_part_fee_msat
            plugin.log(f"MALICIOUS: Sending per-part extra_fee={fee_to_send} (actual={fee_applied})")

        # Build extra_tlvs with (possibly inflated) fee
        fee_byte = fee_to_send.to_bytes(8, "big")
        e = TlvPayload()
        e.add_field(TLV_OPENING_FEE, fee_byte)
...
def setuplsps2service(plugin, peer_id, channel_cap, opening_fee_msat,
                      malicious_extra_fee_msat=None,
                      malicious_per_part_fee_msat=None):
    """Configure the mock LSP service with optional malicious behavior.

    Args:
        peer_id: Target peer to open channel to
        channel_cap: Channel capacity in satoshis
        opening_fee_msat: Actual opening fee in millisatoshis
        malicious_extra_fee_msat: If set, send this inflated fee in TLV instead of actual
        malicious_per_part_fee_msat: If set, send this fee per-part (for sum overflow attack)
    """
    state.target_peer = peer_id
    state.channel_cap = channel_cap
    state.opening_fee_msat = opening_fee_msat
    state.malicious_extra_fee_msat = malicious_extra_fee_msat
    state.malicious_per_part_fee_msat = malicious_per_part_fee_msat

    if malicious_extra_fee_msat:
        plugin.log(f"MALICIOUS MODE: Will send extra_fee={malicious_extra_fee_msat} instead of {opening_fee_msat}")
    if malicious_per_part_fee_msat:
        plugin.log(f"MALICIOUS MODE: Will send per-part fee={malicious_per_part_fee_msat}")
